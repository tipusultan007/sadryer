@extends('tablar::page')

@section('title')
    ক্রয় - বিক্রয় রিপোর্ট
@endsection

@section('content')
    <!-- Page header -->
    <div class="page-header d-print-none">
        <div class="container-xl">
            <div class="row g-2 align-items-center">
                <div class="col">
                    <!-- Page pre-title -->
                    <h2 class="page-title">
                        ক্রয় - বিক্রয় রিপোর্ট
                    </h2>
                </div>
                <!-- Page title actions -->
                <div class="col-12 col-md-auto ms-auto d-print-none">
                    <div class="btn-list">
                        <button class="btn btn-primary d-none d-sm-inline-block" onclick="window.print()">
                            <!-- Download SVG icon from http://tabler-icons.io/i/plus -->
                            <svg xmlns="http://www.w3.org/2000/svg" class="icon" width="24" height="24"
                                 viewBox="0 0 24 24" stroke-width="2" stroke="currentColor" fill="none"
                                 stroke-linecap="round" stroke-linejoin="round">
                                <path stroke="none" d="M0 0h24v24H0z" fill="none"></path>
                                <path
                                    d="M17 17h2a2 2 0 0 0 2 -2v-4a2 2 0 0 0 -2 -2h-14a2 2 0 0 0 -2 2v4a2 2 0 0 0 2 2h2"></path>
                                <path d="M17 9v-4a2 2 0 0 0 -2 -2h-6a2 2 0 0 0 -2 2v4"></path>
                                <path
                                    d="M7 13m0 2a2 2 0 0 1 2 -2h6a2 2 0 0 1 2 2v4a2 2 0 0 1 -2 2h-6a2 2 0 0 1 -2 -2z"></path>
                            </svg>
                            প্রিন্ট করুন
                        </button>
                    </div>
                </div>
            </div>
        </div>
    </div>
    <!-- Page body -->
    <div class="page-body">
        <div class="container-xl">
            @if(config('tablar','display_alert'))
                @include('tablar::common.alert')
            @endif
                @php
                    $date1 = request('date1')?date('d/m/Y',strtotime(request('date1'))):date('d/m/Y');
                    $date2 = request('date2')?date('d/m/Y',strtotime(request('date2'))):date('d/m/Y');
                    if ($date1 === $date2) {
                        $formattedDate = $date1;
                    } else {
                        $formattedDate = $date1 . ' - ' . $date2;
                    }
                @endphp
            <div class="row">
                <div class="col-12 justify-content-center">
                    <div class="info text-center">
                        <h1 class="display-6 fw-bolder mb-1">মেসার্স এস.এ রাইচ এজেন্সী</h1>
                        <span class="badge badge-outline text-gray fs-3">ক্রয় - বিক্রয় রিপোর্ট</span>
                        <h3 class="mt-2">তারিখঃ {{ $formattedDate }}</h3>
                    </div>
                </div>
                    <div class="col-12 d-print-none">
                        <form action="{{ route('report.purchase.sales') }}" method="get">
                            <div class="row">
                                <div class="col-md-2 mb-2">
                                    <input type="text" class="form-control flatpicker" id="date1" name="date1">
                                </div>
                                <div class="col-md-2 mb-2">
                                    <input type="text" class="form-control flatpicker" id="date2" name="date2">
                                </div>
                                <div class="col-md-2 mb-2">
                                    <button class="btn btn-secondary" type="submit">সার্চ করুন</button>
                                </div>
                            </div>
                        </form>
                    </div>
                @php

                    $netSales = $sales - $saleReturns;
                    $netPurchases = $openingStockBalance + $purchases - $purchaseReturns - $closingStockBalance;
                    //$netPurchases = $openingStockBalance + $purchases - $purchaseReturns;
                    $grossProfit = $netSales - $netPurchases;
                    $totalIncome = $grossProfit + $incomes;
                    $netProfit = $totalIncome - $expenses;
                @endphp
                <div class="col-6">
                    <div class="card">
                        <table class="table table-sm card-table table-vcenter table-bordered">
                            <thead class="thead-dark">
                            <tr>
                                <th class="fw-bolder fs-4">বিবরণ</th>
                                <th></th>
                                <th class="fw-bolder fs-4 text-center">টাকা</th>
                            </tr>
                            </thead>
                            <tbody>
                            <tr>
                                <td>বিক্রয়</td>
                                <td class="text-end">{{ number_format($sales) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <td>বিক্রয় ফেরত</td>
                                <td class="text-end">{{ number_format($saleReturns) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <th>নিট বিক্রয়</th>
                                <td></td>
                                <th class="text-end">{{ number_format($netSales) }}</th>
                            </tr>
                            <tr>
                                <td colspan="3"></td>
                            </tr>
                            <tr>
                                <td>শুরুর স্টক ব্যালেন্স</td>
                                <td class="text-end">{{ number_format($openingStockBalance) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <td>ক্রয়</td>
                                <td class="text-end">{{ number_format($purchases) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <td>ক্রয় ফেরত</td>
                                <td class="text-end">{{ number_format($purchaseReturns) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <td>সর্বশেষ স্টক ব্যালেন্স</td>
                                <td class="text-end">{{ number_format($closingStockBalance) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <th>বিক্রয়কৃত পণ্যের মূল্য</th>
                                <td></td>
                                <th class="text-end">{{ number_format($netPurchases) }}</th>
                            </tr>
                            <tr>
                                <td colspan="3"></td>
                            </tr>
                            <tr>
                                <th>মোট মুনাফা</th>
                                <td></td>
                                <th class="text-end">{{ number_format($grossProfit) }}</th>
                            </tr>
                            <tr>
                                <td colspan="3"></td>
                            </tr>
                            <tr>
                                <td>আয়</td>
                                <td class="text-end">{{ number_format($incomes) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <th>মোট আয়</th>
                                <td></td>
                                <th class="text-end">{{ number_format($totalIncome) }}</th>
                            </tr>
                            <tr>
                                <td colspan="3"></td>
                            </tr>
                            <tr>
                                <td>ব্যয়</td>
                                <td class="text-end">{{ number_format($expenses) }}</td>
                                <td></td>
                            </tr>
                            <tr>
                                <th>নিট লাভ</th>
                                <td></td>
                                <th class="text-end">{{ number_format($netProfit) }}</th>
                            </tr>
                            </tbody>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
@endsection

@section('scripts')
    <script type="module">
        document.addEventListener('DOMContentLoaded', function () {
            window.flatpickr("#date1", {
                altInput: true,
                allowInput: true,
                altFormat: "d-m-Y",
                dateFormat: "Y-m-d",
                defaultDate: "{{ request('date1')??date('Y-m-d') }}"
            });
            window.flatpickr("#date2", {
                altInput: true,
                allowInput: true,
                altFormat: "d-m-Y",
                dateFormat: "Y-m-d",
                defaultDate: "{{ request('date2')??date('Y-m-d') }}"
            });
        });
    </script>
@endsection
